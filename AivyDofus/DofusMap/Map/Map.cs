...
namespace AivyDofus.DofusMap.Map
{
    ...
    public class Map
    {
        ...
        private void InitializeMap(byte[] encryptionKey)
        {
            ...
            RelativeId = Raw.ReadUnsignedInt();
            ...
            ShadowBonusOnEntities = Raw.ReadUnsignedInt();

            if (MapVersion >= 9)
            {
                Raw.ReadInt(); // ARGB                
                Raw.ReadUnsignedInt(); // Grid ARGB
            }
            ...
            if (MapVersion >= 4)
            {
                ZoomScale = (ushort)(Raw.ReadUnsignedShort() / 100);
            ...
            if (MapVersion > 10)
            {
                TacticalModelTemplateId = Raw.ReadInt();
            }

            //UseLowPassFilter = Raw.ReadUnsignedByte() == 1;
            //UseReverb = Raw.ReadUnsignedByte() == 1;
